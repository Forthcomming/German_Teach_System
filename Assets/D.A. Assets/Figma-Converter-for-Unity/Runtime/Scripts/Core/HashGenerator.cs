...
{
    [Serializable]
    public class HashGenerator : FcuBase
    {
        ...
        private void GetHashRecursive(FObject target, FObject fobject, List<FObjectHashData> values, int indent)
        {
            ...
            try
            {
                ...
                fobjectHash.FieldHashes.Add(
                    new FieldHashData(
                        nameof(fobject.StrokeWeight),
                        (float)Math.Round(fobject.StrokeWeight, FcuConfig.Rounding.StrokeWeight)
                    )
                );
            ...
            foreach (Paint item in fills)
            {
                try
                {
                    ...
                    if (item.Rotation != null)
                    {
                        effectData.Data.Add(
                            new FieldHashData(
                                nameof(item.Rotation),
                                (float)Math.Round(item.Rotation.Value, FcuConfig.Rounding.Rotation)
                            )
                        );
                    }
                    ...
                    if (item.GradientStops.IsEmpty() == false)
                    {
                        foreach (GradientStop gs in item.GradientStops)
                        {
                            ...
                            effectData.Data.Add(
                                new FieldHashData(
                                    nameof(gs.Position),
                                    (float)Math.Round(gs.Position, FcuConfig.Rounding.Position)
                                )
                            );
                        }
                    }
