...
namespace DA_Assets.FCU
{
    ...
    public class TagSetter : FcuBase
    {
        ...
        private bool SetSmartTags(ref FObject child, ref FObject parent)
        {
            ...
            if (monoBeh.Settings.MainSettings.RawImport == false)
            {
                ...
                if (hasIcon)
                {
                    ...
                    FcuLogger.Debug($"{methodPath} | {string.Join(", ", child.Data.TagReasons)} | {child.Data.NameHierarchy}", FcuDebugSettingsFlags.LogSetTag);
                }
        ...
        {
            int reason = 1;
            string message = "False by default.";
            bool result = false;

            if (fobject.Data.ForceContainer)
            {
                reason = 2;
                message = "ForceContainer is true.";
                result = false;
            }
            else if (fobject.Data.ForceImage)
            {
                reason = 3;
                message = "ForceImage is true.";
                result = false;
            }
            else
            {
                // Start with true - can be inside single image unless a tag says otherwise
                result = true;
                reason = 5;
                message = "All tags allow being inside single image.";
                ...
                foreach (FcuTag fcuTag in fobject.Data.Tags)
                {
                    ...
                    {
                        reason = 4;
                        message = $"Tag {fcuTag} cannot be inside a single image.";
                        result = false;
                        break;
                    }
                }
            ...
            fobject.Data.TagReasons.Add(message);

            FcuLogger.Debug(
                $"Reason: {reason} - {message} | Parent: {parent.Data.NameHierarchy}, Current: {fobject.Data.NameHierarchy}",
