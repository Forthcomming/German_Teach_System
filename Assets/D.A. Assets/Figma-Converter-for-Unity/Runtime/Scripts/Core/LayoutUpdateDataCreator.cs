...
namespace DA_Assets.FCU
{
    ...
    public class LayoutUpdateDataCreator : FcuBase
    {
        ...
        internal async Task<LayoutUpdaterInput> Create(List<FObject> fobjects, List<SyncHelper> syncHelpers, CancellationToken token)
        {
        ...
        private async Task<SelectableObject<DiffInfo>> GetToImport(List<FObject> fobjects, List<SyncHelper> syncHelpers, CancellationToken token)
        {
            ...
            foreach (KeyValuePair<string, DiffInfo> obj in allObjects)
            {
                ...
                if (obj.Value.OldData != null && !obj.Value.NewData.IsDefault())
                {
                    if (di.OldData.GameObject.TryGetComponentSafe(out RectTransform rectTransform))
                    {
                        ...
                        Vector2 Round(Vector2 vec) => vec.Round(FcuConfig.Rounding.DiffCheckerSize);
        ...
        private async Task<SelectableObject<SyncData>> GetToRemove(List<FObject> fobjects, List<SyncHelper> syncHelpers, CancellationToken token)
        {
