...
{
    [Serializable]
    public class DttPuiDrawer : FcuBase
    {
        ...
        public void SetByIndex(int index, object classInstance, Type type, string arrayFieldName, float value)
        {
            ...
            if (fieldInfo == null)
            {
                Debug.LogError(FcuLocKey.log_dtt_field_not_found.Localize(arrayFieldName, type));
                return;
            }
            ...
            if (array == null)
            {
                Debug.LogError(FcuLocKey.log_dtt_field_not_array.Localize(arrayFieldName));
                return;
            }

            if (index < 0 || index >= array.Length)
            {
                Debug.LogError(FcuLocKey.log_dtt_index_out_of_bounds.Localize(index, arrayFieldName));
                return;
            }
            ...
            FcuLogger.Debug($"Value {value} was set at index {index} of array '{arrayFieldName}'.", FcuDebugSettingsFlags.LogComponentDrawer);
        }
