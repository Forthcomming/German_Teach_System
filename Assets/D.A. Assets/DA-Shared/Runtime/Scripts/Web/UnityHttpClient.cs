...
namespace DA_Assets.Networking
{
    public class UnityHttpClient : IDisposable
    {
        ...
        private readonly Dictionary<string, string> _responseHeaders = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        ...
        private UnityHttpClient(HttpMethod method, string url, HttpContent content = null, TimeSpan? timeout = null)
        {
        ...
        public static UnityHttpClient Get(string url, TimeSpan? timeout = null)
        {
            return new UnityHttpClient(HttpMethod.Get, url, null, timeout);
        }

        public static UnityHttpClient Post(string url, WWWForm form, TimeSpan? timeout = null)
        {
            ...
            return new UnityHttpClient(HttpMethod.Post, url, content, timeout);
        }
        ...
        public async Task SendWebRequest(CancellationToken token)
        {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        using (var memoryStream = new MemoryStream())
                        {
                            if (contentLength.HasValue)
                            {
                                ...
                                await responseStream.CopyToAsync(memoryStream, 81920, progress, token).ConfigureAwait(false);
                                downloadProgress = 1f;
                            }
                            else
                            {
#if UNITY_2021_3_OR_NEWER
                                await responseStream.CopyToAsync(memoryStream, token).ConfigureAwait(false);
#else
        ...
        public string GetResponseHeader(string name)
        {
            ...
            _responseHeaders.TryGetValue(name, out string value);
            return value;
        }

        private void CacheResponseHeaders(HttpResponseMessage response)
        {
            if (response == null)
            {
                return;
        ...
        private void CopyHeaders(IEnumerable<KeyValuePair<string, IEnumerable<string>>> headers)
        {
            ...
            {
                _responseHeaders[header.Key] = string.Join(",", header.Value ?? Enumerable.Empty<string>());
            }
        }
    }
