    ...
    public class RequestSender
    {
        public static async Task<Texture2D> LoadImage(string url)
        {
            using (UnityWebRequest webRequest = UnityWebRequestTexture.GetTexture(url))
            {
                ...
                if (isRequestError)
                {
                    Debug.LogError(SharedLocKey.log_request_sender_load_image_error.Localize(webRequest.error));
                    return null;
                }
                else
                {
        ...
        public static async Task<string> Get(string url, int awaitDelayMs = 100)
        {
            using (UnityWebRequest webRequest = UnityWebRequest.Get(url))
            {
                ...
                if (isRequestError)
                {
                    Debug.LogError(SharedLocKey.log_request_sender_get_error.Localize(webRequest.error));
                    return null;
                }
                else
                {
        ...
        public static async Task<string> Post(string url, string postData, string contentType, Dictionary<string, string> headers = null, int awaitDelayMs = 100)
        {
            using (UnityWebRequest webRequest = new UnityWebRequest(url, "POST"))
            {
                ...
                if (isRequestError)
                {
                    ...
                    Debug.LogError(SharedLocKey.log_request_sender_post_error.Localize(methodPath, webRequest.error));
                    return null;
                }
                else
                {
